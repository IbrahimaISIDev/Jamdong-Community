  ...
  return (
    <section className="relative bg-black py-24 px-6 overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-4 sm:gap-6 lg:gap-8 items-center">
          {partners.map((partner, index) => (
            <div
              key={index}
              className="flex items-center justify-center p-3 sm:p-4 bg-gray-900 rounded-xl hover:bg-gray-800 transition-all duration-300 group min-h-[80px] sm:min-h-[100px]"
            >
              <img
                src={partner.logo}
                alt={partner.name}
                className="w-full h-10 sm:h-12 lg:h-14 object-contain opacity-95 group-hover:opacity-100 transition-all duration-300 filter brightness-110 group-hover:brightness-125"
                onError={(e) => {
